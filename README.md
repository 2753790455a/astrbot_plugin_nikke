...
- 单角色练度使用 1200×1500 手机竖版卡，只请求目标角色详情并保留四件装备槽位。

尚未完成或默认禁用：

- 社区签到和奖励领取尚未通过真实写接口验收，写操作默认关闭。
- CDK 兑换已接入官方 BlaBlaLink 接口，但公开配置默认关闭，必须使用授权账号验收后开启。
- 面谈、关卡、企业塔、魔方、收藏品、图片和 JSON 导出不再暴露占位指令。
...
测试覆盖令牌超时与单次消费、Cookie 加密、来源过滤、跨站 CORS、账号隔离基础行为、`game_openid`/正式 `intl_openid` 恢复、AEL 计算、单角色定向查询、四槽装备词条解析、1200×1500 单角色卡和 25 人汇总卡。模拟测试不能替代授权账号的真实端到端验收。
